...
export async function handleGoogleDriveStart(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  if (!await enterprise.userCan(auth.user.userId, workspaceId, "connectors.manage")) {
...
export async function handleGoogleDriveCallback(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  if (!await enterprise.userCan(auth.user.userId, state.workspaceId, "connectors.manage")) {
    return send(res, 403, { error: "connector administration permission required" });
  }
  ...
  const profile = await fetchJson<{ sub?: string; email?: string; name?: string }>(
    "https://openidconnect.googleapis.com/v1/userinfo",
    { headers: { Authorization: `Bearer ${token.access_token}` } },
  );
  if (!profile.sub || !profile.email) return send(res, 502, { error: "Google account lookup failed" });
...
export function drivePermissionAcl(permission: DrivePermission): content.ContentAcl | null {
  if (permission.type === "anyone") return { principalType: "anyone", principalKey: "*" };
  if (permission.type === "domain" && permission.domain) return { principalType: "domain", principalKey: permission.domain };
...
export class GoogleDriveClient {
  ...
  async listPermissions(fileId: string): Promise<DrivePermission[]> {
    ...
    do {
      ...
      url.searchParams.set("supportsAllDrives", "true");
      url.searchParams.set("pageSize", "100");
      url.searchParams.set("fields", "nextPageToken,permissions(id,type,emailAddress,domain,role)");
      if (pageToken) url.searchParams.set("pageToken", pageToken);
...
async function syncFile(input: {
  ...
  const { client, workspaceId, connectorId, file, resources, cache } = input;
  if (file.trashed) {
    await content.markContentDeleted(workspaceId, connectorId, file.id);
    return "deleted";
  }
  ...
  await content.upsertContentItem({
    workspaceId,
    connectorId,
    resourceId: resources.get(resourceExternalId),
    externalId: file.id,
    sourceType: "document",
    title: file.name,
    body,
...
export async function runGoogleDriveSync(job: DriveSyncJob): Promise<void> {
  ...
  try {
    ...
    resources.set("root", root.resourceId);
    for (const drive of await client.listAllDrives()) {
      ...
      resources.set(drive.id, resource.resourceId);
    }
    ...
    const processFile = async (file: DriveFile) => {
      ...
      seen += 1;
      try {
        const result = await syncFile({ client, workspaceId: job.workspaceId, connectorId: job.connectorId, file, resources, cache });
        if (result === "written") written += 1;
        if (result === "deleted") deleted += 1;
      } catch {
        failed += 1;
        await content.markContentDeleted(job.workspaceId, job.connectorId, file.id);
      }
    };
    ...
    await content.finishConnectorSync({
      ...
      errorText: failed > 0 ? `${failed} items failed and were hidden` : "",
    });
...
export async function handleWorkspaceGoogleDrive(input: {
  ...
  if (resource === "syncs" && req.method === "GET" && !sub) {
    send(res, 200, { syncs: await content.latestConnectorSyncs(workspaceId) });
    return true;
  }
